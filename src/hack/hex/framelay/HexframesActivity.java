...
public class HexframesActivity extends Activity {
	...
	public void imgclk(View vi) {
		...
		if (board[cell1][cell2] == 0) {
			...
			// String.valueOf(position) + " on move " + String.valueOf(move),
	...
	public void finishboard(int player) {
		...
		Toast.makeText(getApplicationContext(),"Game won by player  " + String.valueOf(player),Toast.LENGTH_SHORT).show();
		...
		AlertDialog.Builder builder = new AlertDialog.Builder(this);
		  builder.setMessage("Player " + player + "won the match!")
		         .setCancelable(false)
		         .setNeutralButton("Rematch!", new DialogInterface.OnClickListener() {
		...
		AlertDialog alert = builder.create();
		  ...
		  .setTitle("Victory!")
		  .setMessage("Player " + player + " won the match!")
		  .setNeutralButton("Rematch!", new DialogInterface.OnClickListener() {
		  public void onClick(DialogInterface dlg, int sumthin) { 
			  restartFirstActivity2();
		  } })
		  .show();
	}
	...
	public void recurse(int player, int x, int y) {
		...
		if (x % 2 == 1) {
			...
			if (x + 1 <= 6 && board[x + 1][y] == player)
				recurse(player, x + 1, y);
			if (x + 1 <= 6 && y + 1 <= 6 && board[x + 1][y + 1] == player)
				recurse(player, x + 1, y + 1);
		}
		board[x][y] = player;
